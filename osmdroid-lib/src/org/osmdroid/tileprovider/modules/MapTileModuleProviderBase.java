...
public abstract class MapTileModuleProviderBase implements OpenStreetMapTileProviderConstants {
	...
	/**
	 * It is expected that the implementation will construct an internal member which internally implements a
	 * {@link TileLoader}. This method is expected to return a that internal member to methods of the parent methods.
	 * 
	...
	/**
	 * Returns true if implementation uses a data connection, false otherwise. This value is used to determine if this
	 * provider should be skipped if there is no data connection.
	 * 
	...
	public MapTileModuleProviderBase(int pThreadPoolSize, final int pPendingQueueSize) {
		...
		mExecutor = Executors.newFixedThreadPool(pThreadPoolSize, new ConfigurablePriorityThreadFactory(Thread.NORM_PRIORITY,
				getThreadGroupName()));
	...
	// void removeTilesOutOfScreen() {
	// synchronized (mQueueLockObject) {
	// Iterator<Entry<MapTile, MapTileRequestState>> it = mPending.entrySet().iterator();
	// while (it.hasNext()) {
	// Entry<MapTile, MapTileRequestState> e = it.next();
	// if (!e.getKey().isVisible()) {
	// it.remove();
	// }
	// }
	// it = mWorking.entrySet().iterator();
	// while (it.hasNext()) {
	// Entry<MapTile, MapTileRequestState> e = it.next();
	// if (!e.getKey().isVisible()) {
	// it.remove();
	...
	/**
	 * Load the requested tile. An abstract internal class whose objects are used by worker threads to acquire tiles
	 * from servers. It processes tiles from the 'pending' set to the 'working' set as they become available. The key
	 * unimplemented method is 'loadTile'.
	 */
	protected abstract class TileLoader implements Runnable {
		 ...
		 * 
		 * @return the tile if it was loaded successfully, or null if failed to load and other tile providers need to be
		 *         called
		 * @param pState
		...
		/**
		 * A tile has loaded but it's expired. Return it <b>and</b> send request to next provider.
		 */
		...
		final public void run() {
			...
			while ((state = nextTile()) != null) {
				...
				try {
					...
					if (result == null && this instanceof MapTileFilesystemProvider.TileLoader) {
						result = ((MapTileFilesystemProvider.TileLoader) this).getBundledTile(state);
						if (result != null) {
							if (ExpirableBitmapDrawable.isDrawableExpired(result)) {
								state.getCallback().mapTileRequestExpiredTile(state, result);
							} else {
								state.getCallback().mapTileRequestCompleted(state, result);
							}
						}
						// force the download of the tile, because the bundled one is not in the same zoom level
						result = null;
					}
	...
	/**
	 * Thrown by a tile provider module in TileLoader.loadTile() to signal that it can no longer function properly. This
	 * will typically clear the pending queue.
	 */
