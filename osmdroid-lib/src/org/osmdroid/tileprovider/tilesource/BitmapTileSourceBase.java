...
public abstract class BitmapTileSourceBase implements ITileSource, OpenStreetMapTileProviderConstants {
	...
	public BitmapTileSourceBase(final String aName, int aResourceId, final int aZoomMinLevel, final int aZoomMaxLevel,
	...
	public Drawable getDrawable(final String aFilePath) {
		try {
			...
			logger.error("OutOfMemoryError 1 loading bitmap: " + aFilePath);
			System.gc();
		}
		return null;
	}
	...
	public Drawable getDrawable(final InputStream aFileInputStream) throws LowMemoryException {
		try {
			...
			if (bitmap != null) {
				return new ExpirableBitmapDrawable(bitmap);// ReusableBitmapDrawable(bitmap);
			}
		} catch (final OutOfMemoryError e) {
			logger.error("OutOfMemoryError 2 loading bitmap");
			System.gc();
