...
public class MapController implements IMapController, MapViewConstants {
	...
	public void zoomToSpan(final int reqLatSpan, final int reqLonSpan) {
		...
		final int curZoomLevel = this.mOsmv.getZoomLevelFloor();
	...
	public void animateToForTracking(double latitude, double longitude, final double angleRad, final int offsset, boolean applyOffsset) {
		final int x = mOsmv.getScrollX();
		final int y = mOsmv.getScrollY();
		final Point p = TileSystem.LatLongToPixelXY(latitude, longitude, mOsmv.getZoomLevelFloor(), null);
		if (applyOffsset) {
			p.x += Math.sin(angleRad) * offsset;
			p.y += -Math.cos(angleRad) * offsset;
		}
		final int worldSize_2 = TileSystem.MapSize(mOsmv.getZoomLevelFloor()) / 2;
		mOsmv.getScroller().startScroll(x, y, p.x - worldSize_2 - x, p.y - worldSize_2 - y, ANIMATION_DURATION); // ANIMATION_DURATION_DEFAULT
		mOsmv.postInvalidate();

	}

	public void animateToWithOffsset(double latitude, double longitude, final int offsset) {
		final int x = mOsmv.getScrollX();
		final int y = mOsmv.getScrollY();
		final Point p = TileSystem.LatLongToPixelXY(latitude, longitude, mOsmv.getZoomLevelFloor(), null);
		p.y += offsset;
	...
	public void animateTo(final double latitude, final double longitude) {
		...
		final Point p = TileSystem.LatLongToPixelXY(latitude, longitude, mOsmv.getZoomLevelFloor(), null);
		final int worldSize_2 = TileSystem.MapSize(mOsmv.getZoomLevelFloor()) / 2;
		mOsmv.getScroller().startScroll(x, y, p.x - worldSize_2 - x, p.y - worldSize_2 - y, ANIMATION_DURATION);
		mOsmv.getScroller().startScroll(x, y, p.x - worldSize_2 - x, p.y - worldSize_2 - y, ANIMATION_DURATION);
		mOsmv.postInvalidate();
	}

	/**
	 * Animates the underlying {@link MapView} that it centers the passed {@link GeoPoint} in the end. Uses:
	 * {@link MapController.ANIMATION_SMOOTHNESS_DEFAULT} and {@link MapController.ANIMATION_DURATION_DEFAULT}.
	 * 
	 * @param gp
	 */
	...
	/**
	 * Animates the underlying {@link MapView} that it centers the passed {@link GeoPoint} in the end.
	 * 
	 * @param gp
	 *            GeoPoint to be centered in the end.
	 * @param aSmoothness
	 *            steps made during animation. I.e.: {@link MapController.ANIMATION_SMOOTHNESS_LOW},
	 *            {@link MapController.ANIMATION_SMOOTHNESS_DEFAULT}, {@link MapController.ANIMATION_SMOOTHNESS_HIGH}
	 * @param aDuration
	 *            in Milliseconds. I.e.: {@link MapController.ANIMATION_DURATION_SHORT},
	 *            {@link MapController.ANIMATION_DURATION_DEFAULT}, {@link MapController.ANIMATION_DURATION_LONG}
	 */
	...
	/**
	 * Animates the underlying {@link MapView} that it centers the passed coordinates in the end. Uses:
	 * {@link MapController.ANIMATION_SMOOTHNESS_DEFAULT} and {@link MapController.ANIMATION_DURATION_DEFAULT}.
	 * 
	 * @param aLatitudeE6
	 * @param aLongitudeE6
	 ...
	 *            {@link MapController.ANIMATION_DURATION_DEFAULT}, {@link MapController.ANIMATION_DURATION_LONG}
	 */
	public void animateTo(final int aLatitudeE6, final int aLongitudeE6, final AnimationType aAnimationType, final int aSmoothness,
			final int aDuration) {
		...
		case QUARTERCOSINUSALDECELERATING:
			this.mCurrentAnimationRunner = new QuarterCosinusalDeceleratingAnimationRunner(aLatitudeE6, aLongitudeE6, aSmoothness,
					aDuration);
			break;
	...
	public void setCenter(final IGeoPoint point) {
		final Point p = TileSystem.LatLongToPixelXY(point.getLatitudeE6() * 1E-6, point.getLongitudeE6() * 1E-6, mOsmv.getZoomLevelFloor(),
				null);
		// final int worldSize_2 = TileSystem.MapSize(this.mOsmv.getZoomLevel()) / 2;
		final int worldSize_2 = TileSystem.MapSize(this.mOsmv.getZoomLevelFloor()) / 2;
		this.mOsmv.scrollTo(p.x - worldSize_2, p.y - worldSize_2);
		// this.mOsmv.scrollTo(false, p.x - worldSize_2, p.y - worldSize_2);
	}
	...
	@Override
	public float setZoom(final float zoomlevel) {
	...
	private abstract class AbstractAnimationRunner extends Thread {
		...
		public AbstractAnimationRunner(final MapController mapViewController, final int aTargetLatitudeE6, final int aTargetLongitudeE6) {
			this(aTargetLatitudeE6, aTargetLongitudeE6, MapViewConstants.ANIMATION_SMOOTHNESS_DEFAULT,
	...
	private class ExponentialDeceleratingAnimationRunner extends AbstractAnimationRunner {
		...
		public ExponentialDeceleratingAnimationRunner(final int aTargetLatitudeE6, final int aTargetLongitudeE6, final int aSmoothness,
				final int aDuration) {
			super(aTargetLatitudeE6, aTargetLongitudeE6, aSmoothness, aDuration);
	...
	private class CosinusalBasedAnimationRunner extends AbstractAnimationRunner implements MathConstants {
		...
		@SuppressWarnings("unused")
		public CosinusalBasedAnimationRunner(final int aTargetLatitudeE6, final int aTargetLongitudeE6, final float aStart,
		...
		public CosinusalBasedAnimationRunner(final int aTargetLatitudeE6, final int aTargetLongitudeE6, final int aSmoothness,
				final int aDuration, final float aStart, final float aRange, final float aYOffset) {
			super(aTargetLatitudeE6, aTargetLongitudeE6, aSmoothness, aDuration);
	...
	protected class MiddlePeakSpeedAnimationRunner extends CosinusalBasedAnimationRunner implements MathConstants {
		...
		protected MiddlePeakSpeedAnimationRunner(final int aTargetLatitudeE6, final int aTargetLongitudeE6, final int aSmoothness,
				final int aDuration) {
