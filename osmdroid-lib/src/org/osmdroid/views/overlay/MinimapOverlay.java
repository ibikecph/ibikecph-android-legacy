...
/**
 * Draws a mini-map as an overlay layer. It currently uses its own MapTileProviderBasic or a tile provider supplied to
 * it. Do NOT share a tile provider amongst multiple tile drawing overlays - it will create an under-sized cache.
 * 
 * @author Marc Kurtz
 * 
 */
public class MinimapOverlay extends TilesOverlay {
	...
	/**
	 * Creates a {@link MinimapOverlay} with the supplied tile provider. The {@link Handler} passed in is typically the
	 * same handler being used by the main map. The {@link MapTileProviderBase} passed in cannot be the same tile
	 * provider used in the {@link TilesOverlay}, it must be a new instance.
	 * 
	 * @param pContext
	 *            a context
	 ...
	 *            a tile provider
	 */
	public MinimapOverlay(final Context pContext, final Handler pTileRequestCompleteHandler, final MapTileProviderBase pTileProvider,
			final int pZoomDifference) {
	 ...
	 */
	public MinimapOverlay(final Context pContext, final Handler pTileRequestCompleteHandler, final MapTileProviderBase pTileProvider) {
		this(pContext, pTileRequestCompleteHandler, pTileProvider, DEFAULT_ZOOMLEVEL_MINIMAP_DIFFERENCE);
	}

	/**
	 * Creates a {@link MinimapOverlay} that uses its own {@link MapTileProviderBasic}. The {@link Handler} passed in is
	 * typically the same handler being used by the main map.
	 * 
	 * @param pContext
	 *            a context
	 * @param tileRequestCompleteHandler
	...
	protected void drawSafe(final ISafeCanvas pC, final MapView pOsmv, final boolean shadow) {
		...
		final int zoomLevel = projection.getZoomLevelFloor();
		...
		if (zoomLevel - getZoomDifference() < mTileProvider.getMinimumZoomLevel()) {
			miniMapZoomLevelDifference += zoomLevel - getZoomDifference() - mTileProvider.getMinimumZoomLevel();
		}

		// Shift the screen coordinates into the target zoom level
		mTileArea.set(mTileArea.left >> miniMapZoomLevelDifference, mTileArea.top >> miniMapZoomLevelDifference,
				mTileArea.right >> miniMapZoomLevelDifference, mTileArea.bottom >> miniMapZoomLevelDifference);

		// Limit the area we are interested in for tiles to be the MAP_WIDTH by MAP_HEIGHT and
		// centered on the center of the screen
		mTileArea.set(mTileArea.centerX() - (getWidth() / 2), mTileArea.centerY() - (getHeight() / 2), mTileArea.centerX()
				+ (getWidth() / 2), mTileArea.centerY() + (getHeight() / 2));

		// Get the area where we will draw the minimap in screen coordinates
		mMiniMapCanvasRect.set(mViewportRect.right - getPadding() - getWidth(), mViewportRect.bottom - getPadding() - getHeight(),
				mViewportRect.right - getPadding(), mViewportRect.bottom - getPadding());
		...
		pC.drawRect(mMiniMapCanvasRect.left - 2, mMiniMapCanvasRect.top - 2, mMiniMapCanvasRect.right + 2, mMiniMapCanvasRect.bottom + 2,
				mPaint);

		super.drawTiles(pC.getSafeCanvas(), projection.getZoomLevelFloor() - miniMapZoomLevelDifference, TileSystem.getTileSize(),
				mTileArea);
	}
	...
	public boolean onSingleTapUp(final MotionEvent pEvent, final MapView pMapView) {
		...
		if (mMiniMapCanvasRect.contains((int) pEvent.getX() + mViewportRect.left - mWorldSize_2, (int) pEvent.getY() + mViewportRect.top
				- mWorldSize_2)) {
			return true;
		}
