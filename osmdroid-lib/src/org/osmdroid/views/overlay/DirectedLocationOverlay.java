...
public class DirectedLocationOverlay extends Overlay {
	...
	public DirectedLocationOverlay(final Context ctx) {
		this(ctx, new ResourceProxy(ctx));
	}

	public DirectedLocationOverlay(final Context ctx, final ResourceProxy pResourceProxy) {
		super(pResourceProxy);
		this.DIRECTION_ARROW = mResourceProxy.getBitmap(ResourceProxy.direction_arrow);
	...
	public void draw(final Canvas c, final MapView osmv, final boolean shadow) {
		...
		if (this.mLocation != null) {
			...
			final Bitmap rotatedDirection = Bitmap.createBitmap(DIRECTION_ARROW, 0, 0, DIRECTION_ARROW_WIDTH, DIRECTION_ARROW_HEIGHT,
					this.directionRotater, false);
			c.drawBitmap(rotatedDirection, screenCoords.x - rotatedDirection.getWidth() / 2, screenCoords.y - rotatedDirection.getHeight()
					/ 2, this.mPaint);
		}
	}
