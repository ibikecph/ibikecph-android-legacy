...
/**
 * Draws a list of {@link OverlayItem} as markers to a map. The item with the lowest index is drawn as last and
 * therefore the 'topmost' marker. It also gets checked for onTap first. This class is generic, because you then you get
...
public abstract class ItemizedOverlay<Item extends OverlayItem> extends SafeDrawOverlay implements Overlay.Snappable {
	...
	/**
	 * Method by which subclasses create the actual Items. This will only be called from populate() we'll cache them for
	 * later use.
	 */
	...
	protected void drawSafe(ISafeCanvas canvas, MapView mapView, boolean shadow) {
		...
		for (int i = size; i >= 0; i--) {
			...
			if (mapView.getZoomLevel() - (int) mapView.getZoomLevel() == 0.5f) {
				// Log.d("I Bike CPH", "canvas scaled for 1/2");
				canvas.save();
				canvas.scale(0.7f, 0.7f, mCurScreenCoords.x, mCurScreenCoords.y);
				isCanvasDirty = true;
			}
	...
	/**
	 * Utility method to perform all processing on a new ItemizedOverlay. Subclasses provide Items through the
	 * createItem(int) method. The subclass should call this as soon as it has data, before anything else gets called.
	 */
	...
	/**
	 * See if a given hit point is within the bounds of an item's marker. Override to modify the way an item is hit
	 * tested. The hit point is relative to the marker's bounds. The default implementation just checks to see if the
	 * hit point is within the touchable bounds of the marker.
	 * 
	 * @param item
	...
	/**
	 * Override this method to handle a "tap" on an item. This could be from a touchscreen tap on an onscreen Item, or
	 * from a trackball click on a centered, selected Item. By default, does nothing and returns false.
	 * 
	 * @return true if you handled the tap, false if you want the event that generated it to pass to other overlays.
	 */
	...
	/**
	 * Set whether or not to draw the focused item. The default is to draw it, but some clients may prefer to draw the
	 * focused item themselves.
	 */
	...
	/**
	 * If the given Item is found in the overlay, force it to be the current focus-bearer. Any registered
	 * {@link ItemizedOverlay#OnFocusChangeListener} will be notified. This does not move the map, so if the Item isn't
	 * already centered, the user may get confused. If the Item is not found, this is a no-op. You can also pass null to
	 * remove focus.
	 */
	...
	/**
	 * Adjusts a drawable's bounds so that (0,0) is a pixel in the location described by the hotspot parameter. Useful
	 * for "pin"-like graphics. For convenience, returns the same drawable that was passed in.
	 * 
	 * @param marker
