 ...
 * 
 */
public class MyLocationNewOverlay extends SafeDrawOverlay implements IMyLocationConsumer, IOverlayMenuProvider, Snappable {
	...
	public MyLocationNewOverlay(final Context context, IMyLocationProvider myLocationProvider, final MapView mapView) {
	...
	public void setCustomIconCentered(int resId) {
		mPersonBitmap = mResourceProxy.getBitmap(resId);
	...
	protected Rect getMyLocationDrawingBounds(int zoomLevel, Location lastFix, Rect reuse) {
		...
		if (mDrawAccuracyEnabled) {
			...
			final int strokeWidth = (int) FloatMath.ceil(mCirclePaint.getStrokeWidth() == 0 ? 1 : mCirclePaint.getStrokeWidth());
	...
	/**
	 * Enable receiving location updates from the provided IMyLocationProvider and show your location on the maps. You
	 * will likely want to call enableMyLocation() from your Activity's Activity.onResume() method, to enable the
