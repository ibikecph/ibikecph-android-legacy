...
/**
 * Base class representing an overlay which may be displayed on top of a {@link MapView}. To add an overlay, subclass
 * this class, create an instance, and add it to the list obtained from getOverlays() of {@link MapView}.
 * 
 * This class implements a form of Gesture Handling similar to
 * {@link android.view.GestureDetector.SimpleOnGestureListener} and {@link GestureDetector.OnGestureListener}. The
 * difference is there is an additional argument for the item.
 * 
...
public abstract class Overlay implements OverlayConstants {
	...
	public Overlay(final Context ctx) {
		mResourceProxy = new ResourceProxy(ctx);
	...
	/**
	 * Sets whether the Overlay is marked to be enabled. This setting does nothing by default, but should be checked
	 * before calling draw().
	 */
	 ...
	 * Overlays should use this method to obtain and store a menu id for each menu item at construction time. This will
	 * ensure that two overlays don't use the same id.
	 * 
	...
	/**
	 * Similar to <see cref="getSafeMenuId" />, except this reserves a sequence of IDs of length <param name="count" />.
	...
	/**
	 * Draw the overlay over the map. This will be called on all active overlays with shadow=true, to lay down the
	 * shadow layer, and then again on all overlays with shadow=false. Callers should check isEnabled() before calling
	 * draw(). By default, draws nothing.
	 */
	...
	/**
	 * Convenience method to draw a Drawable at an offset. x and y are pixel coordinates. You can find appropriate
	 * coordinates from latitude/longitude using the MapView.getProjection() method on the MapView passed to you in
	 * draw(Canvas, MapView, boolean).
	 * 
	 * @param shadow
	 *            If true, draw only the drawable's shadow. Otherwise, draw the drawable itself.
	 */
	protected synchronized static void drawAt(final android.graphics.Canvas canvas, final android.graphics.drawable.Drawable drawable,
	...
	public interface Snappable {
		 ...
		 * @param snapPoint
		 *            To be filled with the the interesting point (in screen coordinates) that is closest to the given x
		 *            and y. Can be untouched if not snapping.
		 * @param mapView
		 *            The {@link MapView} that is requesting the snap. Use MapView.getProjection() to convert between
