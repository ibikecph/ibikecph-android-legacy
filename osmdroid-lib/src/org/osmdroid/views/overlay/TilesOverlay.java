...
public class TilesOverlay extends SafeDrawOverlay implements IOverlayMenuProvider {
	...
	protected void drawSafe(final ISafeCanvas c, final MapView osmv, final boolean shadow) {
		...
		// MapTile.mViewPort = mViewPort;

		// Draw the tiles!
		drawTiles(c.getSafeCanvas(), pj.getZoomLevelFloor(), TileSystem.getTileSize(), mViewPort);

	}

	/**
	 * This is meant to be a "pure" tile drawing function that doesn't take into account osmdroid-specific
	 * characteristics (like osmdroid's canvas's having 0,0 as the center rather than the upper-left corner). Once the
	 * tile is ready to be drawn, it is passed to onTileReadyToDraw where custom manipulations can be made before
	 * drawing the tile.
	 */
	public void drawTiles(final Canvas c, final int zoomLevel, final int tileSizePx, final Rect viewPort) {
	...
	private final TileLooper mTileLooper = new TileLooper() {
		...
		public void handleTile(final Canvas pCanvas, final int pTileSizePx, final MapTile pTile, final int pX, final int pY) {
			...
			// if (mapView != null && pTile.getZoomLevel() != mapView.zoomLevelPinch) {
			// final int zoomdiff = pTile.getZoomLevel() - mapView.zoomLevelPinch;
			// if (zoomdiff > 0) {
			// tile = new MapTile(mapView.zoomLevelPinch, pTile.getX() >> zoomdiff, pTile.getY() >> zoomdiff);
			// } else if (zoomdiff < 0) {
			// tile = new MapTile(mapView.zoomLevelPinch, pTile.getX() << -zoomdiff, pTile.getY() << -zoomdiff);
			// }
			// }

			final int zoomdiff = pTile.getZoomLevel() - 17;
			if (pTile.getZoomLevel() > 17) {
				tile = new MapTile(17, pTile.getX() >> zoomdiff, pTile.getY() >> zoomdiff);
			}
			tile.originalTile = pTile;
			...
			if (currentMapTile != null) {
				...
				// mTileRect.set(pX * pTileSizePx, pY * pTileSizePx, pX * pTileSizePx + pTileSizePx, pY * pTileSizePx +
				// pTileSizePx);
	...
	public boolean onCreateOptionsMenu(final Menu pMenu, final int pMenuIdOffset, final MapView pMapView) {
		final SubMenu mapMenu = pMenu.addSubMenu(0, MENU_MAP_MODE + pMenuIdOffset, Menu.NONE, mResourceProxy.getString(R.string.map_mode))
				.setIcon(mResourceProxy.getDrawable(ResourceProxy.ic_menu_mapmode));
		...
		final String title = pMapView.getResourceProxy().getString(
				pMapView.useDataConnection() ? R.string.offline_mode : R.string.online_mode);
	...
	public boolean onPrepareOptionsMenu(final Menu pMenu, final int pMenuIdOffset, final MapView pMapView) {
		...
		pMenu.findItem(MENU_OFFLINE + pMenuIdOffset).setTitle(
				pMapView.getResourceProxy().getString(pMapView.useDataConnection() ? R.string.offline_mode : R.string.online_mode));
	 ...
	 *            the color to use. If the value is {@link Color.TRANSPARENT} then there will be no loading tile.
	 */
	...
	/**
	 * Set this to overshoot the tile cache. By default the TilesOverlay only creates a cache large enough to hold the
	 * minimum number of tiles necessary to draw to the screen. Setting this value will allow you to overshoot the tile
	 * cache and allow more tiles to be cached. This increases the memory usage, but increases drawing performance.
	 * 
