...
public class ItemizedIconOverlay<Item extends OverlayItem> extends ItemizedOverlay<Item> {
	...
	private boolean rescaleWhenZoomed = false;

	public ItemizedIconOverlay(final List<Item> pList, final Drawable pDefaultMarker,
	...
	public ItemizedIconOverlay(final List<Item> pList,
			final org.osmdroid.views.overlay.ItemizedIconOverlay.OnItemGestureListener<Item> pOnItemGestureListener,
			final ResourceProxy pResourceProxy) {
		this(pList, pResourceProxy.getDrawable(ResourceProxy.marker_default), pOnItemGestureListener, pResourceProxy);
	}

	public ItemizedIconOverlay(final Context pContext, final List<Item> pList,
			final org.osmdroid.views.overlay.ItemizedIconOverlay.OnItemGestureListener<Item> pOnItemGestureListener) {
		this(pList, new ResourceProxy(pContext).getDrawable(ResourceProxy.marker_default), pOnItemGestureListener, new ResourceProxy(
				pContext));
	}
	...
	/**
	 * Each of these methods performs a item sensitive check. If the item is located its corresponding method is called.
	 * The result of the call is returned.
	 * 
	 * Helper methods are provided so that child classes may more easily override behavior without resorting to
	...
	/**
	 * When a content sensitive action is performed the content item needs to be identified. This method does that and
	 * then performs the assigned task on that item.
	 * 
	 * @param event
	 * @param mapView
	 * @param task
	...
	private boolean activateSelectedItems(final MotionEvent event, final MapView mapView, final ActiveItem task) {
		...
		for (int i = 0; i < this.mItemList.size(); ++i) {
			...
			if (hitTest(item, marker, mTouchScreenPoint.x - mItemPoint.x, mTouchScreenPoint.y - mItemPoint.y)) {
				if (task.run(i)) {
					return true;
				}
			}
	...
	public boolean isRescaleWhenZoomed() {
		return rescaleWhenZoomed;
	}

	public void setRescaleWhenZoomed(boolean rescaleWhenZoomed) {
		this.rescaleWhenZoomed = rescaleWhenZoomed;
	}
