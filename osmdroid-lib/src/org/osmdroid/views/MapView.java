...
public class MapView extends ViewGroup implements IMapView, MapViewConstants, MultiTouchObjectCanvas<Object>, TileDownloadListener {
	...
	public MapView(final Context context, final int tileSizePixels, final ResourceProxy resourceProxy,
			final MapTileProviderBase aTileProvider, final Handler tileRequestCompleteHandler) {
	...
	/**
	 * You can add/remove/reorder your Overlays using the List of {@link Overlay}. The first (index 0) Overlay gets
	...
	public BoundingBoxE6 getBoundingBox(final int pViewWidth, final int pViewHeight) {
		...
		return new BoundingBoxE6(neGeoPoint.getLatitudeE6(), neGeoPoint.getLongitudeE6(), swGeoPoint.getLatitudeE6(),
	...
	public Rect getIntrinsicScreenRect(final Rect reuse) {
		...
		out.set(getScrollX() - getWidth() / 2, getScrollY() - getHeight() / 2, getScrollX() + getWidth() / 2, getScrollY() + getHeight()
				/ 2);
		return out;
	}

	/**
	 * Get a projection for converting between screen-pixel coordinates and latitude/longitude coordinates. You should
	 * not hold on to this object for more than one draw, since the projection of the map could change.
	 * 
	 * @return The Projection of the map in its current state. You should not hold on to this object for more than one
	 *         draw, since the projection of the map could change.
	 */
	@Override
	...
	void setMapCenter(final int aLatitudeE6, final int aLongitudeE6) {
		...
		if (getAnimation() == null || getAnimation().hasEnded()) {
			mScroller.startScroll(getScrollX(), getScrollY(), coords.x - worldSize_2 - getScrollX(), coords.y - worldSize_2 - getScrollY(),
					500);
			postInvalidate();
		}
	}
	...
	float setZoomLevel(final float aZoomLevel, IGeoPoint gp) {
		...
		if (newZoomLevelFloor > curZoomLevelFloor) {
			...
			// getScrollY()
			// + worldSize_current_2, curZoomLevelFloor, null);
			final Point centerPoint = TileSystem.LatLongToPixelXY(centerGeoPoint.getLatitudeE6() * 1E-6,
					centerGeoPoint.getLongitudeE6() * 1E-6, newZoomLevelFloor, null);
	...
	/**
	 * Zoom the map to enclose the specified bounding box, as closely as possible. Must be called after display layout
	 * is complete, or screen dimensions are not known, and will always zoom to center of zoom level 0. Suggestion:
	 * Check getScreenRect(null).getHeight() > 0
	 */
	public void zoomToBoundingBox(final BoundingBoxE6 boundingBox) {
		...
		final double maxZoomLatitudeSpan = mZoomLevel == getMaxZoomLevel() ? currentBox.getLatitudeSpanE6() : currentBox
				.getLatitudeSpanE6() / Math.pow(2, getMaxZoomLevel() - mZoomLevel);
		...
		final double maxZoomLongitudeSpan = mZoomLevel == getMaxZoomLevel() ? currentBox.getLongitudeSpanE6() : currentBox
				.getLongitudeSpanE6() / Math.pow(2, getMaxZoomLevel() - mZoomLevel);
	...
	/**
	 * Set the map to limit it's scrollable view to the specified BoundingBoxE6. Note this does not limit zooming so it
	 * will be possible for the user to zoom to an area that is larger than the limited area.
	 * 
	 * @param boundingBox
	...
	public void invalidateMapCoordinates(Rect dirty) {
		...
		if (this.getMapOrientation() != 0)
			GeometryMath.getBoundingBoxForRotatatedRectangle(mInvalidateRect, centerX, centerY, this.getMapOrientation() + 180,
					mInvalidateRect);
	...
	/**
	 * Returns a set of layout parameters with a width of {@link android.view.ViewGroup.LayoutParams#WRAP_CONTENT}, a
	 * height of {@link android.view.ViewGroup.LayoutParams#WRAP_CONTENT} at the {@link GeoPoint} (0, 0) align with
	...
	public boolean dispatchTouchEvent(final MotionEvent event) {
		...
		if (mMultiTouchController != null && mMultiTouchController.onTouchEvent(event)) {
			if (DEBUGMODE) {
				logger.debug("multiTouchController handled touch");
			}
	...
	/**
	 * Check mAnimationListener.isAnimating() to determine if view is animating. Useful for overlays to avoid
	...
	public static class LayoutParams extends ViewGroup.LayoutParams {
		...
		/**
		 * Special value for the alignment requested by a View. TOP_LEFT means that the location will at the top left
		 * the View.
		 */
		public static final int TOP_LEFT = 1;
		/**
		 * Special value for the alignment requested by a View. TOP_RIGHT means that the location will be centered at
		 * the top of the View.
		 */
		public static final int TOP_CENTER = 2;
		/**
		 * Special value for the alignment requested by a View. TOP_RIGHT means that the location will at the top right
		 * the View.
		 */
		public static final int TOP_RIGHT = 3;
		/**
		 * Special value for the alignment requested by a View. CENTER_LEFT means that the location will at the center
		 * left the View.
		 */
		public static final int CENTER_LEFT = 4;
		/**
		 * Special value for the alignment requested by a View. CENTER means that the location will be centered at the
		 * center of the View.
		 */
		public static final int CENTER = 5;
		/**
		 * Special value for the alignment requested by a View. CENTER_RIGHT means that the location will at the center
		 * right the View.
		 */
		public static final int CENTER_RIGHT = 6;
		/**
		 * Special value for the alignment requested by a View. BOTTOM_LEFT means that the location will be at the
		 * bottom left of the View.
		 */
		public static final int BOTTOM_LEFT = 7;
		/**
		 * Special value for the alignment requested by a View. BOTTOM_CENTER means that the location will be centered
		 * at the bottom of the view.
		 */
		public static final int BOTTOM_CENTER = 8;
		/**
		 * Special value for the alignment requested by a View. BOTTOM_RIGHT means that the location will be at the
		 ...
		 *            the location of the child within the map view
		 * @param alignment
		 *            the alignment of the view compared to the location {@link #BOTTOM_CENTER}, {@link #BOTTOM_LEFT},
		 *            {@link #BOTTOM_RIGHT} {@link #TOP_CENTER}, {@link #TOP_LEFT}, {@link #TOP_RIGHT}
		 * @param offsetX
		 ...
		 */
		public LayoutParams(final int width, final int height, final IGeoPoint geoPoint, final int alignment, final int offsetX,
				final int offsetY) {
			super(width, height);
			if (geoPoint != null) {
		...
		/**
		 * Since we cannot use XML files in this project this constructor is useless. Creates a new set of layout
		 * parameters. The values are extracted from the supplied attributes set and context.
		 * 
		 * @param c
